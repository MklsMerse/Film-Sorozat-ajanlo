...
namespace FilmFokuszBackEnd.Controllers
{
    ...
    public class FilmekController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post(string token, Filmek filmek)
        {
        ...
        public async Task<IActionResult> Put(string token, Filmek filmek)
        {
            if (Program.LoggedInUsers.ContainsKey(token) && Program.LoggedInUsers[token].PermissionId == 9)
            {
                try
        ...
        public async Task<IActionResult> GetId(int id, string token)
        {
            if (Program.LoggedInUsers.ContainsKey(token) && Program.LoggedInUsers[token].PermissionId == 9)
            {
                try
                {
                    using (var cx = new FilmfokuszContext())
                    {
                        return Ok(await cx.Filmeks.FirstOrDefaultAsync(f => f.FilmId == id));
                    }
                }
                catch (Exception ex)
                {
