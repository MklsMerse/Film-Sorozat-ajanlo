...
namespace FilmFokuszBackEnd.Controllers
{
    ...
    public class SorozatokController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post(string token, Sorozatok sorozatok)
        {
        ...
        public async Task<IActionResult> Put(string token, Sorozatok sorozatok)
        {
            if (Program.LoggedInUsers.ContainsKey(token) && Program.LoggedInUsers[token].PermissionId == 9)
            {
                try
                {
                    using (var cx = new FilmfokuszContext())
                    {
                        cx.Sorozatoks.Update(sorozatok);
                        cx.SaveChanges();
        ...
        public async Task<IActionResult> GetId(int id, string token)
        {
            if (Program.LoggedInUsers.ContainsKey(token) && Program.LoggedInUsers[token].PermissionId == 9)
            {
                try
                {
                    using (var cx = new FilmfokuszContext())
                    {
                        return Ok(await cx.Sorozatoks.FirstOrDefaultAsync(f => f.SorozatId == id));
                    }
                }
                catch (Exception ex)
                {
